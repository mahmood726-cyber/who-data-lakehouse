...
PAGE_URL = "https://www.who.int/data/data-collection-tools/who-mortality-database"


class MortalityDatabaseClient:
    ...
    def discover_downloads(self) -> dict[str, str]:
        ...
        for link in soup.find_all("a", href=True):
            ...
            if "mortality-raw-data" not in href or not filename.lower().endswith(".zip"):
                continue
            downloads[filename] = href
        if not downloads:
            raise RuntimeError("No WHO mortality raw ZIP downloads discovered")
        return downloads
